/**
 * Re-link an existing personal account as Acme org admin (idempotent).
 *
...
async function relink() {
  ...
  const result = await sql`
    ...
    WHERE o.slug = 'acme' AND u.email = ${email}
  ...
  console.log(
    result.length
      ? `Linked ${email} as Acme org admin (org_id ${result[0].org_id}).`
      : `No change for ${email}.`
  );
}
