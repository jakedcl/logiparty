...
async function seed() {
  ...
  await sql`
    ...
    WHERE o.slug = 'testtenant3pl' AND u.email = 'admin@acme.test'
  ...
  await sql`
    ...
    FROM organizations o, users u
    WHERE o.slug = 'testtenant3pl' AND u.email = 'morgan@acme.test'
    ON CONFLICT (org_id, user_id) DO NOTHING
  `;
  await sql`
    INSERT INTO org_memberships (org_id, user_id, is_org_admin, is_manager, is_staff, is_client)
    ...
    FROM organizations o, users u
    WHERE o.slug = 'testtenant3pl' AND u.email IN ('sam@acme.test', 'dana@acme.test')
    ON CONFLICT (org_id, user_id) DO NOTHING
  `;
  await sql`
    INSERT INTO org_memberships (org_id, user_id, is_org_admin, is_manager, is_staff, is_client)
  ...
  await sql`
    ...
    WHERE o.slug = 'testtenant3pl' AND u.email = 'sam@acme.test'
  ...
  await sql`
    ...
    JOIN users u ON u.id = m.user_id
    JOIN organizations o ON o.id = m.org_id
    WHERE o.slug = 'testtenant3pl' AND u.email = 'dana@acme.test'
    ON CONFLICT DO NOTHING
  `;
