...
To wipe seeded filler (`nydac` + `test` + `axis` orgs + legacy demo/acme seed users, catalogs) and re-seed fresh:

```bash
npm run db:reset-seed -- --confirm
```

Uses `DATABASE_URL` from `.env.local`. Only deletes seed orgs (`nydac`, `test`, `axis`, and legacy `demo`/`acme`/…) plus seed test accounts — other users are kept. After re-seed, **`nydac`**, **`test`**, and **`axis`** exist. **Does not delete the whole database** — but if Production shares this URL, Production sees the same change. Script refuses the Neon production hostname.
